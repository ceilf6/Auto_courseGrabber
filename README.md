...
- **多门替换**：替换课程支持填写多个课程号（逗号分隔），脚本会**依次退选**所有替换课程后再抢新课。适合需要同时腾出多个时间段的场景（如退掉两门课才能选上目标课）
- **⚠️注意**：由于抢 课A 前需要先把替换课程全部退了，若退选后目标课已无空位则等待下次机会；任意一门替换课退选失败时本次抢课中止
...
5. 可选：设置替换课程（如：`23306047` 或多门 `23306047,23306048`，逗号分隔，选中新课前依次退选）
6. 点击"➕ 添加课程"按钮
...
const TARGET_COURSES = [
    ...
    },
    {
        code: '23306048',
        priority: 3,
        replaceCodes: ['23306047']          // 选上此课前先退选一门
    },
    {
        code: '23306049',
        priority: 4,
        replaceCodes: ['23306047', '23306050']  // 选上此课前依次退选两门
    },
];
```
